...
<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    <h1 class="h3 mb-1">Duplicate FSC/Barcodes</h1>
    <p class="text-muted mb-0">Review and fix products that share the same FSC or barcode</p>
  </div>
  <div>
...
<?php if (!empty($skuGroups)): ?>
  <div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-white border-bottom"><h5 class="mb-0"><i class="bi bi-upc-scan me-2"></i>Duplicate FSCs</h5></div>
    <div class="card-body">
      <?php foreach ($skuGroups as $g): $key = $g['sku']; $items = $itemsByKey['sku'][$key] ?? []; ?>
        <div class="mb-4">
          <div class="d-flex align-items-center mb-2">
            <span class="badge bg-warning me-2">FSC: <?= htmlspecialchars($key) ?></span>
            <span class="text-muted small"><?= count($items) ?> products</span>
          </div>
          <div class="table-responsive">
            <table class="table table-sm align-middle">
              <thead class="table-light"><tr><th>ID</th><th>Title</th><th>FSC</th><th>Barcode</th><th>Price</th><th>Stock</th><th class="text-end">Actions</th></tr></thead>
              <tbody>
